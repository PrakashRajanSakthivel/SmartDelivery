...
export class RestaurantMenuComponent implements OnInit {
  ...
  addToCart(menuItem: MenuItem): void {
    ...
    const currentRestaurantId = this.cartService.getRestaurantId();
    const hasItems = this.cartService.getItems().length > 0;

    if (hasItems && currentRestaurantId && currentRestaurantId !== this.restaurant.id) {
  ...
  getItemQuantity(menuItemId: string): number {
    const cartItem = this.cartService.getCartValue().find(ci => ci.menuItemId === menuItemId);
  ...
  decrementItem(item: MenuItem): void {
    ...
    this.cartService.updateItemQuantity(item.id, qty - 1);
  }
